...
# Now build a model using smda with caret using the training set:
model <- train(diagnosis~., method="smda", data= trainingData)
model
...
model <- train(diagnosis~., method="superpc", data= trainingData, trControl=control)
model